...
### 6.3a — Run-loop wiring + model_versions (consumes 6.1a + 6.1b + 6.2) ✅ 2026-08-29

| Do | Files | Done when |
|---|---|---|
| `attach_analytics`/`run_frame`: per-camera `EventEngine` between `analytics.process()` and sinks; envelope JSONL sink at `runs/events/<cam>.envelopes.jsonl` (raw candidate log stays). `model_versions` assembled at composition from loaded artifacts (detector/pose model names from detection config, action onnx name + short SHA) and stamped into every envelope | `edge/mobisentra/pipeline.py`, `edge/mobisentra/events/sink.py` (envelope writer), `edge/mobisentra/main.py` (`--severity-config`) + `edge/tests/test_event_wiring.py` | ✅ **9/9 wiring tests + real-CLI smoke** (suite 306→315, ruff clean). Stub-detector tests: occupancy flow through the REAL severity policy + engine → envelopes land in the file and every line validates (envelope + event schemas); `model_versions` stamped from detection config; raw rows keep their pre-envelope shape (kind fields, no severity/specversion). Startup guards: missing severity.yaml → `SystemExit` "not found"; invalid policy → `SystemExit` with the validator message. **Production smoke** (real stack: yolo26n-pose on mps, bus1.mp4, event-friendly registry — capacity 2/confirm 2/loiter 2s): **60 raw rows → 55 envelopes, 5 debounce-suppressed; all 55 schema-valid; kinds/severities correct** (occupancy_level_change LOW ×44, overcrowding HIGH ×8 — ratio ≥ 1.5 escalation live, restricted_zone_entry LOW ×3); `source=/mobisentra/edge/SMOKE_BUS/SMOKE_CAM`, `model_versions={'detector': 'yolo26n-pose.pt'}`. Design notes: (1) **action version not stamped in production yet** — the fight scorer factory is benchmark/soak-only (Phase 5 wiring); `build_model_versions(det_cfg, action_onnx)` ships with the `name@sha8` stamping implemented + unit-tested for the day fight production wiring lands (recorded as an open enablement decision, not silently dropped); (2) one `JsonlEventWriter` class serves both sinks (`write()` widened to `Mapping[str, object]`); (3) `--severity-config` CLI flag defaults to `configs/severity.yaml` (same pattern as `--detection-config`); (4) smoke lesson: `bus_interior_01.mp4` has ~no people in frame — use `bus1.mp4` (3–6 people) for event-producing smoke runs |

### 6.3b — Evidence writer generalization (independent) ✅ 2026-08-29
