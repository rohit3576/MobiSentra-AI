...
| 8 | Backend Services | restart-safe consumer; event < 1 s to dashboard | 🟡 In progress — 8.1a ✅ 8.1b ✅ 8.2 ✅ + **8.3a ✅** (PG `events`+`audit_log` migrations, no-ORM runner w/ `schema_migrations`, divergent-history refusal; 9/9, backend 40/40 + tsc clean; **live-applied to compose PG**: applied 2 → rerun skipped 2); next 8.3c Socket.IO |
