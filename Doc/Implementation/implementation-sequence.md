...
| 8 | Backend Services | restart-safe consumer; event < 1 s to dashboard | 🟡 In progress — 8.1a ✅ 8.1b ✅ 8.2 ✅ 8.3a ✅ 8.3c ✅ + **8.3b ✅** (write-path pipeline: dedupe→PG ON CONFLICT→Redis live state→WS, A1 analytics+occupancy both paths, PG-throws/Redis-degrades/poison-skips semantics; 20 new tests, backend 65/65 + tsc clean; live smoke: PG conflict net proven, merged camera hash, analytics zero-PG); next 8.4a REST API (+A2 evidence route) |
