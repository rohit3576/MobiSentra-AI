...
| 8 | Backend Services | restart-safe consumer; event < 1 s to dashboard | 🟡 In progress — 8.1a ✅ 8.1b ✅ 8.2 ✅ 8.3a ✅ + **8.3c ✅** (Socket.IO `alerts:{vehicle_id}` rooms, ack'd subscribe w/ input guard, `EventPusher` boundary for the pipeline; 5/5 timing-free isolation tests, backend 45/45 + tsc clean); next 8.3b write-path pipeline |
