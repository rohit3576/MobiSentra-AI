...
|---|---|---|---|
| 5.1a weights + manifest | download script for HF `engares/MoViNet4Violence-Detection` (A2 default, A0 fallback), SHA256 + size recorded; SOURCES.md row: origin, unlicensed-repo status, benchmark-only, wrapper-only posture; gitignored like UR Fall | `mlops/datasets/download_movinet.py`, `mlops/datasets/SOURCES.md` | script downloads + SHA-verifies A2; SOURCES.md row exists; `git status` clean of weights |
| 5.1b runtime spike | measure the ladder on MPS with A2 over a 20–30 s sample stream: per-frame latency + end-to-end FPS impact; CPU-only sanity run; record numbers in this doc | results table → this doc | runtime picked on measured numbers; choice + numbers recorded below in **Step 5.1 decisions** |
| 5.1c wrapper | `ActionScorer` behind a pure interface (DetectorTracker pattern — downstream code never imports TF); streaming mode (state carried across frames); warm-up semantics documented; unit tests run against a recorded-score stub (no TF import in the test path) | `edge/mobisentra/vision/action.py` + tests | streaming scores over a real sample clip via our interface; stubbed unit tests green; latency/FPS impact measured |
...
| runtime (tflite / TF / ONNX) | *pending 5.1b* | *measured MPS latency + FPS impact* |
