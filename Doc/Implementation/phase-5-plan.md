...
> **Status: EXECUTED 2026-08-28 · CLOSED 2026-08-29 (owner decision (c)
> "accept documented limitation" — Gate-4 FP-fork precedent; see
> `phase-5-completion.md`). Tuning deferred to the Phase 10 data loop.**
...
| Sub | Do | Done when |
|---|---|---|
| 5.5a runner ✅ 2026-08-28 | `tools/fight_benchmark.py` (extends the fall_benchmark pattern): UBI-Fights frame-level GT windows = primary; Hockey = trigger-stage-only (2 s clips, protocol documented); negatives soak = FP rate | ✅ **selftest PASS** (stub tracker + stub scorer + synthetic clips — no models, no datasets, CI-able): fight clip → 1 alert + pair formed + peak 0.67; calm clip → 0 alerts; JSON metrics emitted. Protocols implemented: UBI (GT-window detection ±1 s, FP/hr on 784 normal clips), Hockey trigger-stage (pair formation + peak score — sustain chain cannot fit 2 s, documented), negatives FP. Tuning knobs: `--action-min/--rel-motion/--sustain-s`. Engine gained `last_action_scores` telemetry for peak-score reporting |
| 5.5b datasets ✅ 2026-08-28 — **no owner input needed after all** | Kaggle creds became unnecessary: **UBI-Fights came from the official host** (the 2026-08-27 "page unreachable" was an EXPIRED SSL CERT — fetch with verification relaxed; integrity pinned by SHA256) and **Hockey via the Academic Torrents mirror** (aria2c torrent, no creds). Download facts: UBI zip 8,117,112,419 B — single connection ≈ 72 KB/s, aria2c 16-way ranges ≈ 4.5–10 MiB/s; Hockey zip 171,330,668 B exact, piece-verified. Layout discoveries: UBI videos split `fight/`+`normal/`, annotations ONE LABEL PER LINE (parser handles both forms), official 67-clip test split (40F/27N) used; Hockey = 1000 flat `fi*/no*_xvid.avi`. Both gitignored, SHA256s + citations in SOURCES.md (Degardin & Proença IJCB 2020; Nievas et al. CAIP 2011) | ✅ both datasets verifiable on disk; `git status` clean of dataset files |
| 5.5c run + verdict ✅ 2026-08-29 (closed) | full run: detection rate on UBI-Fights fight windows + Hockey trigger-stage; FP = alerts over the 30-min negatives set; fill the Gate 5 table | **Baseline numbers** (evidence: `runs/fight-benchmark-salvage.json`, `runs/fight-benchmark-windowed-PARTIAL.json`, `runs/fight-negatives-soak.json`): UBI test-split fights **11/40 detected (full-length) / 5/35 (windowed ±5s/+2s)** — far under the ≥85% gate; breakdown: most misses are SILENT in-window (no alert at all — model/pair side: small/distant/movable-camera fights), 3 fired-outside-GT-window (tolerance issue); UBI normals 0 alerts on the 4 measured; negatives corpus 22 alerts/42.4 min (31/hr vs 0 gate; assist+crowd clean). Perf fixed en route: `shared_session_factory` (0.8×→6.0× realtime, identical outputs); windowed protocol added for fast iteration. **Verdict: Gate 5 NOT met — owner decision (c) 2026-08-29: accept documented limitation** (Gate-4 FP-fork precedent); tuning deferred to Phase 10 data loop, windowed protocol (~5 min/run) reserved for any later iteration. Report: `phase-5-completion.md` |
