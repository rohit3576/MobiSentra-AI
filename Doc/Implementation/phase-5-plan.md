...
| 5.1c wrapper | `ActionScorer` behind a pure interface (DetectorTracker pattern — downstream code never imports TF); onnxruntime engine; streaming state carry (73 tensors); warm-up semantics documented; unit tests run against a recorded-score stub (no runtime import in the test path) | `edge/mobisentra/vision/action.py` + tests | streaming scores over a real sample clip via our interface; stubbed unit tests green; latency/FPS impact measured |
