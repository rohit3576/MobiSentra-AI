...
### 8.5 — Integration suite (compose stack; gated) ✅ 2026-08-31

| Do | Files | Done when |
|---|---|---|
| Vitest suite against the live stack (gated on stack reachability, skip-with-instructions like the edge pattern): publish N envelopes straight to Kafka → assert PG rows + Redis state + WS client receipts; **latency probe** (publish→WS-receive < 1 s, the Gate-8 criterion); **restart mid-stream**: stop the consumer process mid-batch → restart → zero lost/duplicated PG rows (at-least-once redeliveries collapsed by dedupe + ON CONFLICT); one full-edge-path case via `tools/messaging_check.py` | `backend/test/integration/backend.test.ts` + `test/integration/helpers.ts` (+ `src/index.ts` — the one-process assembly this step finally exercises: migrations on boot → API+WS on one port → consumer supervisor w/ backoff + graceful SIGTERM) | ✅ **suite 4/4 green inside the full run (backend 87/87, tsc clean, no orphaned processes); gated skip PROVEN (kafka stopped → clean skip + instructions); full-edge-path PASS** (`messaging_check.py` run `1144d0c5`: 3/3 through edge publisher → EMQX → bridge → Kafka → backend → PG, 11.5 s round-trip, rows verified + cleaned). **Gate-8 numbers:** latency probe worst publish→WS-receipt **23 ms** (runs: 19–28 ms; criterion < 1,000 — ~40× headroom); restart mid-stream **SIGKILLed at 5–27/150 in PG → final 150 rows / 150 distinct, exact id-set match, zero double-push** (149/150 live receipts — one push in flight at the kill instant, expected); ack+escalate over HTTP → **2 audit rows** + acked/escalation columns verified. **Live findings (the step's real work):** (1) the kafka-javascript `.d.ts` LIES for the callback-only `consume()` — runtime flow-mode delivers **single messages** (not `Message[]`) and forwards **idle timeouts (code −185) as errors**; driver now guards both (single-or-array + timeout filter) — trust the lib source, not the types; (2) `pnpm exec` does **not** forward SIGTERM to children — every harness stop/kill signals the **process group** (`detached` + `kill(-pid)`); orphaned consumers silently steal the group partition and poison WS-receipt assertions (found via group-membership describe); (3) `session.timeout.ms` tuned to 6 s (broker floor; heartbeat 2 s) — failover in seconds instead of 45; (4) a **warmup-receipt readiness gate** in `beforeAll` — group rebalance after churn takes seconds; nothing is measured until partition+pipeline+push are proven live; (5) fresh group on `earliest` replays the topic backlog by design — first boot ingested **1,162 Phase-7 history events into dev PG**, wire-dupes collapsed by the dedupe net (live `duplicate` outcomes) |
...
## Gate 8 — checklist (from the runbook) — **PASSED 3/3, 2026-08-31**

- [x] Consumer restart mid-stream: zero lost/duplicated rows — SIGKILL at 5–27/150 landed → final 150/150 distinct, exact id set, zero double-push (8.5 evidence)
- [x] Dashboard receives an event < 1 s after edge publish — worst publish→WS-receipt 23 ms (≈40× headroom); full-edge-path round-trip 11.5 s end-to-end incl. MQTT+bridge
- [x] Ack/escalate write audit rows — asserted over live HTTP (2 audit rows + acked/escalation columns)
