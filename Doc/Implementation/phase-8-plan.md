...
| Do | Files | Done when |
|---|---|---|
| `consumer/pipeline.ts`: validate → map → dedupe → `INSERT … ON CONFLICT (event_id) DO NOTHING` → Redis live state (`camera:{id}` hash: last event, severity, ts; TTL 5 min) → WS room emit; every store injectable; returns per-stage outcome for observability. **A1 (approved with phase-9 plan 2026-08-30): analytics branch** — `org.mobisentra.analytics.*` envelopes (validated by `schemas/events/v0/analytics.schema.json`, extending the Phase-0 ajv set) → Redis live state ONLY (`occupancy_ratio`, `people_count`, ts; **no PG row** — `events.severity` is NOT NULL, analytics aren't incidents) + `state` message on room `cameras:{vehicle_id}`; 8.3c's `EventPusher` gains `publishState` with its own room-isolation test | `backend/src/consumer/pipeline.ts` + `test/pipeline.test.ts` | unit (all fakes): fresh event → PG insert + Redis update + WS emit; duplicate id → single PG row, no re-emit; PG down → error propagates (no commit → redelivery); Redis live-state failure → event still lands in PG (degrade, log); **A1: analytics envelope → Redis update + `cameras:` WS state, zero PG writes** |

### 8.4a — REST API (fastify)

| Do | Files | Done when |
|---|---|---|
| GET `/api/incidents` (filters: severity, camera, vehicle, time window; newest-first), GET `/api/incidents/:id` (detail + evidence_ref), GET `/api/cameras` (distinct from PG + live status from Redis: online if state TTL alive), GET `/api/events` (raw history, cursor paging), POST `/api/incidents/:id/ack` + `/escalate` (write `events` ack columns **and** `audit_log` row); fastify + typed handlers, pg pool injected. **A2 (approved with phase-9 plan 2026-08-30): `GET /api/evidence/*`** serving files from `EVIDENCE_ROOT` (env; demo default = edge evidence output dir, confirmed at 9.4), resolving `local://evidence/…` refs from envelopes — **path-traversal-sandboxed** (resolve + prefix check), `video/mp4` | `backend/src/api/server.ts` + `test/api.test.ts` (fastify inject + fake stores) | unit: all 6 endpoints happy-path + filters + 404s; ack/escalate → audit rows present; **A2: evidence route serves a fixture file + rejects `../` traversal payloads with 403/404**; live against compose in 8.5 |
