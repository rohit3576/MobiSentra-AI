...
| Do | Files | Done when |
|---|---|---|
| `consumer/dedupe.ts`: `SET dedupe:{source}:{id} NX EX 86400` wrapper — returns first-seen true/false; injectable redis client (ioredis in prod, fake in tests) | `backend/src/consumer/dedupe.ts` + `test/dedupe.test.ts` | unit: first → process, repeat → skip; TTL passed through; redis down → fail-safe choice = **process anyway** (PG ON CONFLICT still guards; a dedupe outage must not drop events — documented) |
