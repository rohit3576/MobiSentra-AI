...
| [`phase-8-plan.md`](./phase-8-plan.md) + [`phase-8-completion.md`](./phase-8-completion.md) | Phase 8 (backend services) — **EXECUTED + CLOSED 2026-08-31, Gate 8 PASSED 3/3** (23 ms latency, SIGKILL-mid-stream 150/150 zero-loss, audit rows; full-edge-path PASS ×2; backend 87/87; compose service live-verified) — reports carry the live-caught lessons (.d.ts vs runtime at native-lib boundaries, orphaned-consumer sabotage, backlog replay by design) |
| [`phase-9-plan.md`](./phase-9-plan.md) | Phase 9 (dashboard → **v0.1.0**) design plan — **APPROVED 2026-08-30** (auth dropped; occupancy thresholds, Redis-only analytics, demo-replay compose profile locked); two Phase-8 amendments folded into the 8.3b/8.4a rows (analytics branch, evidence route); execution starts after Gate 8; 9.1a–9.5 + 9.7 division |
