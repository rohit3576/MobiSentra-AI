...
| [`phase-7-plan.md`](./phase-7-plan.md) | Phase 7 (edge messaging: spool → MQTT → bridge → Kafka) design plan — **DRAFT, awaiting approval**; division 7.1a–7.4b (SQLite write-ahead spool, QoS-1 publisher w/ injectable transport, bridge hardening incl. its own README backlog, fault-injection + 10-min gate soak); no owner-input steps; defaults proposed |
